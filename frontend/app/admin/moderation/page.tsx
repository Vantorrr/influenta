            ...
            <Card key={r.id}>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex gap-2 pt-2">
                  <Button size="sm" variant="primary" onClick={async () => { await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/users/${r.id}/verify`, { method: 'PATCH', headers: { 'Authorization': `Bearer ${localStorage.getItem('influenta_token')}` } }); try { window.dispatchEvent(new Event('refreshModerationCount')) } catch {} try { window.dispatchEvent(new CustomEvent('user-verified', { detail: { userId: r.id } })) } catch {} try { (window as any).Telegram?.WebApp?.HapticFeedback?.notificationOccurred?.('success') } catch {} alert('Пользователь верифицирован'); setRequests(prev => prev.filter(x => x.id !== r.id)) }}>Верифицировать</Button>
